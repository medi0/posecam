...
export function applyDigiFilter(
...
export function captureFrame(
  ...
  if (overlayImage && overlayState.opacity > 0) {
    ...
    ctx.scale(
      overlayState.flipH ? -overlayState.scale : overlayState.scale,
      overlayState.scale,
    );
...
export function analyzeComposition(img: HTMLImageElement) {
  ...
  const subject =
    leftBright > rightBright + 15
      ? "Left third"
      : rightBright > leftBright + 15
        ? "Right third"
        : "Center";

  const horizon =
    topBright > botBright + 20
      ? "Lower third"
      : botBright > topBright + 20
        ? "Upper third"
        : "Middle";

  const cameraHeight =
    topBright > botBright + 30
      ? "High angle"
      : botBright > topBright + 30
        ? "Low angle"
        : "Eye level";

  const edgeBright = (topBright + botBright) / 2;
  const cameraAngle =
    Math.abs(leftBright - rightBright) > 25 ? "Tilted" : "Straight";
