          ...
          <div className="flex-1 overflow-y-auto no-scrollbar space-y-2">
            {templates.length === 0 ? (
              <p className="text-white/40 text-xs text-center py-6">
                No templates yet. Set up your overlay, grid, and filter the way
                you like, then save it for next time.
              </p>
            ) : (
              templates.map((t) => (
                <div
                  ...
                  className={`flex items-center gap-3 rounded-xl p-2.5 transition-colors ${
                    activeTemplateId === t.id
                      ? "bg-accent-500/25 ring-1 ring-accent-400/50"
                      : "bg-white/8"
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-white/40 text-[10px]">
                      {t.gridMode !== "none" ? t.gridMode : "no grid"} ·{" "}
                      {t.digiFilter !== "none" ? t.digiFilter : "natural"}
                    </p>
                  </div>
                  <button
