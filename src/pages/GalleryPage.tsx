...
function PhotoCard({ photo, burstCount, onDelete, onView }: {
  photo: CapturedPhoto; burstCount?: number; onDelete: () => void; onView: () => void
}) {
  return (
...
export function GalleryPage() {
  ...
  const activePhoto = lightbox ? photos.find(p => p.id === lightbox.photoId) : null
  const activeSiblings = activePhoto?.burstGroup ? burstSiblings.get(activePhoto.burstGroup) || [] : []
  const siblingIndex = activePhoto ? activeSiblings.findIndex(p => p.id === activePhoto.id) : -1
  ...
  const stepSibling = (dir: -1 | 1) => {
    ...
    const next = (siblingIndex + dir + activeSiblings.length) % activeSiblings.length
            ...
            {activeSiblings.length > 1 && lightbox.view === 'result' && (
              <div className="flex items-center justify-center gap-1.5">
                {activeSiblings.map((s, i) => (
                  <button
                    key={s.id}
                    onClick={() => setLightbox({ photoId: s.id, view: 'result' })}
