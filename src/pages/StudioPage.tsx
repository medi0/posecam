...
import {
  Upload, SwitchCamera, ChevronUp, ChevronDown,
  Trash2, Eye, EyeOff, Bookmark, FlipHorizontal2
} from 'lucide-react'
...
export function StudioPage() {
  ...
  const [activePanel, setActivePanel] = useState<'controls' | 'templates'>('controls')
  const [isFlashing, setIsFlashing] = useState(false)

  const {
    overlay, gridMode, digiFilter, showControls,
    setOverlayImage, addPhoto, setCompositionHints, toggleControls,
    mirrorPreview, mirrorCapture,
    timerDuration, isCountingDown, countdownValue,
    burstCount, burstInterval, isBursting, burstProgress,
    setIsBursting, setBurstProgress,
  } = usePoseCamStore()
  ...
  // Takes a single frame and saves it. mirrorCapture/saveOverlayInPhoto are
  // read fresh from the store each time rather than captured in a closure,
  // since burst sequences run over multiple seconds.
  const takeSingleShot = useCallback((burstGroup?: string) => {
    ...
    const { cleanDataUrl, compositedDataUrl } = captureFrame(
      videoRef.current,
      overlayImg,
      state.overlay,
      state.digiFilter,
    ...
    // saveOverlayInPhoto OFF (default): the saved photo is camera-only.
    // ON: the saved photo has the overlay baked in, matching old behavior.
    // cleanDataUrl is always kept too, so Gallery can show Before/After
    // regardless of this setting.
    addPhoto({
      id: `${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      dataUrl: state.saveOverlayInPhoto ? compositedDataUrl : cleanDataUrl,
      cleanDataUrl,
      inspirationUrl: state.overlay.image,
      timestamp: Date.now(),
      burstGroup,
      overlayBaked: state.saveOverlayInPhoto,
    })
  }, [videoRef, addPhoto])
  ...
  return (
    <div className="relative w-full h-full bg-black overflow-hidden" ref={containerRef}>
      ...
      {/* Capture flash */}
      {isFlashing && (
        <div className="absolute inset-0 bg-white z-50 pointer-events-none" style={{ opacity: 0.75 }} />
      )}
      ...
      {isBursting && burstCount > 1 && (
        <div className="absolute top-16 left-0 right-0 z-40 flex justify-center pointer-events-none">
          <div className="glass-dark rounded-full px-4 py-1.5 flex items-center gap-1.5">
            ...
            <span className="text-white/70 text-[10px] font-medium ml-1">{burstProgress}/{burstCount}</span>
          </div>
        </div>
      )}
      ...
      <div className="absolute top-0 left-0 right-0 z-30 p-4 flex items-center justify-between">
        <div className="glass-dark rounded-full px-3 py-1.5 flex items-center gap-1.5">
          ...
          {mirrorPreview && <FlipHorizontal2 size={11} className="text-accent-300" />}
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => { setActivePanel('templates'); setShowPanel(true) }}
      ...
      <div className="absolute bottom-20 left-0 right-0 z-30 flex flex-col items-center gap-3">
        ...
        >
          <div className={`rounded-full border-[3px] w-12 h-12 ${isBursting ? 'border-accent-400' : 'border-gray-300'}`} />
        </button>
        {(timerDuration > 0 || burstCount > 1) && !isCountingDown && !isBursting && (
